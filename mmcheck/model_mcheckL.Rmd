...
perf[1] <- summary(lm(Y~predict(fit.ls)))$r.squared
...
abline(a = 0, b = 1, col=2)
perf[2] <- summary(lm(as.vector(beta) ~ as.vector(fit.seqT$C) ))$r.squared
...
plot(Uta,Ua)
abline(a = 0, b = 1, col=2)
perf[3] <- summary(lm(Uta~Ua))$r.squared
...
set.seed(SD)
rank.est <- 5
fit.L[[i]] <- gofar_s(scale(X1, center = F, scale = F),
                    scale(cbind(Q,Z), center = F, scale = F), cIndex = 1:c,
                   nrank = rank.est, family = family,
                   nlambda = nlam, familygroup = familygroup,
                   control = control, nfold = 5
...
plot(Y,predict(fit.ls))
abline(a = 0, b = 1, col=2)
PF[i,1] <- summary(lm(Y~predict(fit.ls)))$r.squared # 0.9799  # 0.4488
...
# predict(fit.ls)
plot(as.vector(beta[,selInd< coff[i]]),as.vector(fit.seqMS$C))
abline(a = 0, b = 1, col=2)
PF[i,2] <- summary(lm(as.vector(beta[,selInd< coff[i]]) ~ as.vector(fit.seqMS$C) ))$r.squared # 0.9063, 0.3627
...
# plot(Uta,Ua)
# abline(a = 0, b = 1, col=2)
if(!is.na(sum(Ua))) PF[i,3] <- summary(lm(Uta~Ua))$r.squared # 0.9766, 0.0798
}


# save(fit.L, file = 'mmcheck_ms.rds')
# load( 'mmcheck_ms.rds')
...
PF_I[i,4] <- sum(selIndZ<coff[i])
# Model fitting: GOFAR(S) (full data)
selIndZ <- apply(beta,1,norm,'2')
set.seed(SD)
rank.est <- 5
fit.LI[[i]]  <- gofar_s(scale(X, center = F, scale = F),
                    scale(cbind(Q,Z[,selIndZ<coff[i]]), center = F, scale = F), cIndex = 1:c,
                   nrank = rank.est, family = family,
                   nlambda = nlam, familygroup = familygroup,
                   control = control, nfold = 5
)
...
U <- Z[,selIndZ<coff[i]] %*% fit.seqMSI$U %*% diag(fit.seqMSI$D)
fit.ls <- lm(Y ~ U+Q)
# predict(fit.ls)
plot(Y,predict(fit.ls))
abline(a = 0, b = 1, col=2)
PF_I[i,1] <- summary(lm(Y~predict(fit.ls)))$r.squared  # 0.9799  # 0.4488 # 0.444



# predict(fit.ls)
plot(as.vector(beta[selIndZ<coff[i],]),as.vector(fit.seqMSI$C))
abline(a = 0, b = 1, col=2)
PF_I[i,2] <- summary(lm(as.vector(beta[selIndZ<coff[i],]) ~ as.vector(fit.seqMSI$C) ))$r.squared  # 0.9063, 0.3627 0.5495



Ua <- U%*%coef(fit.ls)[1+(1:ncol(U))]
Uta <- Z[,selIndZ<coff[i]] %*% betaU[selIndZ<coff[i],] %*% alpha
# plot(Uta,Ua)
# abline(a = 0, b = 1, col=2)
if(!is.na(sum(Ua))) PF_I[i,3] <- summary(lm(Uta~Ua))$r.squared  # 0.9766, 0.0798, 0.257

}

rbind(PF_I,c(perf,ncol(Z)) )

# save(fit.LI, file = 'mmcheck_msi.rds')
